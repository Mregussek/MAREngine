...
namespace mar {
	namespace gui {
		...
		void GUI::initialize(window::Window* window, const char* glsl_version) {
			_window = window;
		...
		void GUI::connectToRenderer(graphics::Renderer* renderer) {
			_renderer = renderer;
		...
		void GUI::addNewObjectToScene() {
			...
			if (ImGui::Button("Select Pyramid")) {
				if (_guiData.centers.size() == graphics::constants::maxObjectsInScene)
					return;
		...
		void GUI::display_StatisticsMenu() {
			if (_rendererConnected) {
				graphics::RendererStatistics stats = _renderer->getStatistics();
